...
export const CaseRepository = {
    ...
    findAllWithAggregatedData: () => {
        const cases = CaseRepository.findAll();
        const evidenceRows = db.prepare('SELECT case_id, source, importance, tags FROM evidence WHERE deleted_at IS NULL').all();
        const caseEvidenceMap = new Map();
        ...
        return cases.map((caseItem) => {
            ...
            for (const ev of evidences) {
                ...
                if (rank > highestImportanceRank) {
                    ...
                    highestImportance = ev.importance;
                }
            }
            return {
                ...caseItem,
    ...
    search: (filters) => {
        ...
        if (filters.tags.length > 0) {
            cases = cases.filter((c) => filters.tags.some((tag) => c.allTags.includes(tag)));
        }
        if (filters.sources.length > 0) {
            cases = cases.filter((c) => filters.sources.some((source) => c.allSources.some((s) => s === source || s.includes(source))));
        }
        if (filters.importance) {
            const targetRank = importanceOrder[filters.importance] || 0;
            cases = cases.filter((c) => {
                if (!c.highestImportance)
                    return false;
    ...
    getAllAvailableTags: () => {
        const rows = db.prepare('SELECT DISTINCT tags FROM evidence WHERE deleted_at IS NULL AND tags != \'[]\'').all();
    ...
    getAllAvailableSources: () => {
        const rows = db.prepare('SELECT DISTINCT source FROM evidence WHERE deleted_at IS NULL AND source IS NOT NULL AND source != \'unknown\'').all();
